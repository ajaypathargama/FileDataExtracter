...
function App() {
  ...
  const progressInstance = <ProgressBar animated now={progress} label={`${progress.toFixed(0)}%`} />;
  ...
  const uploadChunk = async (chunk) => {
    try {
      const response = await axios.post("https://localhost:44356/fileupload/UploadChunks", chunk, {
        params: {
          id: counter,
          fileName: fileGuid,
        },
  ...
  const uploadCompleted = async () => {
    ...
    const response = await axios.post("https://localhost:44356/fileupload/UploadComplete", {}, {
      params: {
        fileName: fileGuid,
      },
      data: formData,
  ...
  return (
    <Jumbotron>
      <Form>
        <div>Select csv file to upload it on server</div><br></br>
        <Form.Group>
